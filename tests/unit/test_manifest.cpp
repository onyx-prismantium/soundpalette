...
void check_dim_stats(const json& stats) {
    for (const char* dim : {"bright01", "warm01", "ton01", "atk01", "tail01", "loud01", "jitter01"}) {
...
void check_file_entry(const json& fe) {
    ...
    for (const char* key :
         {"hue_deg", "sat", "light", "size_px", "spike01", "spikes", "jitter01", "tail01", "seed"}) {
...
TEST_CASE("manifest/schema") {
    ...
    for (const char* key : {"schema_version", "mapping_version", "root", "file_count", "files", "stats"}) {
